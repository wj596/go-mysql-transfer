...
import (
	...
	"go-mysql-transfer/endpoint/lua/module"
...
func New(endpointType uint32) *lua.LState {
	...
	module.PreloadLogModule(L)            //加载log模块
	module.PreloadDatabaseClientModule(L) //加载database模块
	module.PreloadRedisClientModule(L)    //加载redis客户端

	switch endpointType {
	case constants.EndpointTypeRedis:
		//module.PreloadRedisEndpointModule(L) //加载redis端点
