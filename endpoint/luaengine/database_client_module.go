...
func preloadDatabaseClientModule(L *lua.LState) {
	L.PreloadModule("dbclient", dbClientModuleLoader)
}
...
func get(L *lua.LState) int {
	...
	dsFormat := LvToString(L.GetGlobal(GlobalDataSourceName))
	...
	for rows.Next() {
		...
		for i, v := range values {
			if v != nil {
				vv := commons.RawBytesToInterface(v, types[i].DatabaseTypeName())
				PaddingLuaTableWithValue(L, result, columns[i], vv)
			}
		}
	}
...
func query(L *lua.LState) int {
	...
	for rows.Next() {
		...
		for i, v := range values {
			vv := commons.RawBytesToInterface(v, types[i].DatabaseTypeName())
			PaddingLuaTableWithValue(L, item, columns[i], vv)
		}
